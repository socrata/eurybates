...
final class Envelope private (val tag: String, val details: JValue) {
  ...
  def forced: Envelope = new Envelope(tag, details.forced)

  override def toString: String = s"Envelope($tag, $details)"
}
...
object Envelope {
  ...
  private[eurybates] def fromParts(tag: String, value: JValue): Envelope =
